...
export default function SolicitudDetalleCliente() {
  ...
  async function subirComprobante() {
    if (!archivoComprobante) return toast.error('Seleccioná un archivo.');
    setSubiendo(true);
    const form = new FormData();
    form.append('comprobante', archivoComprobante);
    try {
      await api.post(`/solicitudes/${id}/adjuntos`, form, {
    ...
    <div className="space-y-5 max-w-3xl">
      ...
      {solicitud.estadoActual === 'OBSERVADO' && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
          <p className="font-medium text-amber-800">⚠️ Tu solicitud tiene observaciones</p>
          {solicitud.historialEstados?.filter(h => h.estado === 'OBSERVADO' && h.motivoInterno === null)
            .slice(-1).map(h => (
              <p key={h.id} className="text-sm text-amber-700 mt-1">{h.motivoInterno}</p>
            ))}
          <p className="text-sm text-amber-600 mt-2">Revisá los comentarios a continuación para más información.</p>
        </div>
      )}
      ...
      {['PENDIENTE', 'EN_REVISION'].includes(solicitud.estadoActual) && (
        <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
          <h2 className="font-semibold text-gray-800">Adjuntar comprobante de pago</h2>
          <p className="text-sm text-gray-500">Si aún no adjuntaste el comprobante, podés hacerlo aquí (PDF, JPG o PNG, máx. 5 MB).</p>
          <input
            type="file"
      ...
      {/* Comentarios visibles al cliente */}
      ...
      {/* Renovar certificado vencido/próximo a vencer */}
